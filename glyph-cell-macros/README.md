...
be mixed in the same invocation, for example one ASCII block and one CJK block.
The macro deduplicates repeated characters inside one block, rejects duplicate
characters across blocks, rasterizes with FreeType, and emits the rasterized
bitmap plus raw glyph bounds, advance width, generated cell width, and generated
bitmap offset for runtime layout.
